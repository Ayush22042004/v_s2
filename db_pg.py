...
def migrate_and_seed():
    schema = '''
        CREATE TABLE IF NOT EXISTS users(
            id SERIAL PRIMARY KEY, username TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT NOT NULL CHECK (role IN ('admin','voter','candidate'))
        );
    ...
    CREATE TABLE IF NOT EXISTS candidates(
      id SERIAL PRIMARY KEY, name TEXT NOT NULL, category TEXT NOT NULL,
            election_id INTEGER REFERENCES elections(id) ON DELETE CASCADE, votes INTEGER DEFAULT 0, user_id INTEGER REFERENCES users(id) ON DELETE SET NULL
    );
